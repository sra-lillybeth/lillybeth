export const hu = {
  // Routes (URL segments for this language)
  routes: {
    accommodation: 'szallas',
    rooms: 'szobak',
    booking: 'foglalas',
  },

  // Header
  header: {
    nav: {
      home: 'Főoldal',
      accommodations: 'Szállások',
      rooms: 'Szobák',
      contact: 'Kapcsolat',
    },
    bookNow: 'Foglalás',
  },

  // Hero Section
  hero: {
    headline: 'Lillybeth Vendégházak',
    subtitle: 'Ahol a pihenés már a tervezésnél elkezdődik',
    scrollDown: 'Görgessen a felfedezéshez',
  },

  // Search Section
  search: {
    title: 'Találja meg tökéletes szállását',
    checkIn: 'Érkezés',
    checkOut: 'Távozás',
    guests: 'Vendégek',
    guestSingular: 'Vendég',
    guestPlural: 'Vendég',
    searchButton: 'Keresés',
    selectDate: 'Válasszon dátumot',
    selectCheckIn: 'Válassza ki az érkezés dátumát',
    selectCheckOut: 'Válassza ki a távozás dátumát',
    clearDates: 'Dátumok törlése',
    searching: 'Keresés...',
    combinationsTitle: 'Elérhető szobakombinációk',
    bestMatch: 'Legjobb ajánlat',
    totalRooms: 'Szobák',
    totalCapacity: 'Összes férőhely',
    selectCombination: 'Kiválasztás',
    selectBestMatch: 'Legjobb ajánlat kiválasztása',
    modifySearch: 'Keresés módosítása',
    noResults: 'Nincs elérhető szoba',
    noResultsDescription: 'Nem találtunk szobakombinációt {count} vendég számára a kiválasztott időszakban.',
    tryDifferentDates: 'Próbáljon más dátumot',
    adjustGuestCount: 'Módosítsa a vendégszámot',
    filterByAccommodation: 'Szűrés szállás szerint',
    anyAccommodation: 'Bármely szállás',
    selectAccommodation: 'Válasszon ki legalább egy szállást az eredmények megtekintéséhez',
    showAll: 'Összes megjelenítése',
  },

  // Buildings Section
  buildings: {
    title: 'Épületeink',
    subtitle: 'Minden épület egyedi élményt kínál a Balatonnál',
    viewAll: 'Összes épület',
    viewDetails: 'Részletek',
    learnMore: 'Tudjon meg többet',
    capacity: 'Max. {count} fő',
    capacitySingular: '1 fő',
  },

  // About Section
  about: {
    title: 'A Lillybeth-ről',
    subtitle: 'Prémium pihenés a Balatonnál',
    content: `Üdvözöljük a Lillybeth Vendégházakban, ahol a foglalás gyors, egyszerű és kényelmes. Már az első kattintástól érezni fogja: jó helyen jár.

Foglaljon közvetlenül nálunk, garantáltan a legjobb áron, rejtett költségek nélkül – közvetlen kapcsolatban a szállásadóval.

Hiszünk abban, hogy a kivételes vendégélmény már a tervezésnél elkezdődik – időt szánunk a Balaton szépségének megbecsülésére, élvezzük a csendes pillanatokat, és egy életen át tartó emlékeket teremtünk.`,
  },

  // Gallery Section
  gallery: {
    title: 'Galéria',
    subtitle: 'Pillanatok a jövőbeli menedékéből',
    viewAll: 'Összes fotó',
  },

  // Map Section
  map: {
    title: 'Itt talál minket',
    subtitle: 'Helyszíneink várják látogatását',
    getDirections: 'Útvonaltervezés',
  },

  // Footer
  footer: {
    tagline: 'Békés menedéke vár',
    contact: {
      title: 'Kapcsolat',
      email: 'E-mail',
      phone: 'Telefon',
    },
    address: {
      title: 'Cím',
    },
    quickLinks: {
      title: 'Gyors linkek',
      privacy: 'Adatvédelmi irányelvek',
      terms: 'Általános szerződési feltételek',
      cancellation: 'Lemondási feltételek',
    },
    copyright: '© {year} Lillybeth. Minden jog fenntartva.',
  },

  // Common
  common: {
    loading: 'Betöltés...',
    error: 'Hiba történt',
    tryAgain: 'Próbálja újra',
    close: 'Bezárás',
    previous: 'Előző',
    next: 'Következő',
    backToHome: 'Vissza a főoldalra',
  },

  // Lightbox
  lightbox: {
    close: 'Galéria bezárása',
    previous: 'Előző kép',
    next: 'Következő kép',
    imageOf: '/',
  },

  // Accommodation Detail Page
  accommodation: {
    title: 'Szállás',
    titlePlural: 'Szállások',
    description: 'Leírás',
    amenities: 'Felszereltség',
    houseRules: 'Házirend',
    bookingConditions: 'Foglalási feltételek',
    location: 'Helyszín',
    roomTypes: 'Elérhető szobák',
    checkAvailability: 'Elérhetőség ellenőrzése',
    showMore: 'Több mutatása',
    showLess: 'Kevesebb mutatása',
    fromPrice: 'Induló ár',
    perNight: '/ éjszaka',
    viewDetails: 'Részletek',
    bookThisRoom: 'Szoba foglalása',
    capacity: 'Max. {count} fő',
    capacitySingular: '1 fő',
    noRoomsAvailable: 'Jelenleg nincs elérhető szoba ennél a szállásnál.',
    backToAccommodations: 'Vissza a szállásokhoz',
  },

  // Room Types
  roomTypes: {
    title: 'Szobatípusok',
    price: 'Ár',
    capacity: 'Férőhely',
    amenities: 'Felszereltség',
    viewRoom: 'Szoba megtekintése',
    bookRoom: 'Szoba foglalása',
  },

  // Booking Page
  booking: {
    title: 'Foglalás véglegesítése',
    guestInfo: 'Vendég adatai',
    guestName: 'Teljes név',
    guestNamePlaceholder: 'Adja meg teljes nevét',
    email: 'E-mail cím',
    emailPlaceholder: 'az.on@email.com',
    phone: 'Telefonszám',
    phonePlaceholder: '+36 XX XXX XXXX',
    arrivalTime: 'Várható érkezési idő',
    notes: 'Különleges kérések',
    notesPlaceholder: 'Bármilyen különleges kérés vagy megjegyzés...',
    summary: 'Foglalás összegzése',
    additionalServices: 'Kiegészítő szolgáltatások',
    mandatory: 'Kötelező',
    accommodationTotal: 'Szállás',
    additionalTotal: 'Kiegészítő szolgáltatások',
    total: 'Összesen',
    completeBooking: 'Foglalás véglegesítése',
    processing: 'Feldolgozás...',
    secureBooking: 'Foglalása biztonságos és azonnal megerősített',
    confirmationTitle: 'Foglalás megerősítve!',
    confirmationMessage: 'Köszönjük foglalását. Megerősítő e-mailt küldtünk az Ön címére.',
    referenceNumber: 'Hivatkozási szám',
    clearCart: 'Összes törlése',
    selectDates: 'Dátum kiválasztása',
  },

  // Thank You Page
  thankYou: {
    whatsNext: 'Mi a következő lépés?',
    emailSent: 'Megerősítő e-mailt küldtünk a postafiókjába',
    reviewDetails: 'Tekintse át a foglalás részleteit az e-mailben',
    contactUs: 'Lépjen kapcsolatba velünk, ha kérdése van',
    questions: 'Kérdése van?',
    contactSupport: 'Lépjen kapcsolatba ügyfélszolgálatunkkal',
  },
};
