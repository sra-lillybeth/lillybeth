...
export function AccommodationHeroSlider({
  ...
  // Format capacity text (max guests)
  const getGuestsText = () => {
    if (!maxGuests) return null;
    if (maxGuests === 1) return t.accommodation.capacitySingular;
    return t.accommodation.capacity.replace('{count}', maxGuests.toString());
  };
    ...
    >
      ...
      {images.map((image, index) => (
        ...
        >
          ...
          {/* Additional gradient for bottom depth */}
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-white/90">
            ...
            {maxRooms != null && maxRooms > 0 && (
              <div className="flex items-center gap-2 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                ...
                <span className="text-sm md:text-base">{maxRooms} {t.booking.roomsPlural}</span>
              </div>
            )}
            ...
            {maxGuests != null && maxGuests > 0 && (
              <div className="flex items-center gap-2 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                ...
                <span className="text-sm md:text-base">{getGuestsText()}</span>
              </div>
            )}
          </div>
        </div>
